...
const ChatBot: React.FC<ChatBotProps> = memo(({ universityContext: propUniversityContext, assessmentData: propAssessmentData, initialMessage: propInitialMessage, forceNewConversation = false, userContext, resumeConversationId, resumeConversationTitle }) => {
  const { 
    currentConversation, 
    addMessage, 
    addConversation,
  ...
  useEffect(() => {
    ...
    if (!currentConversation) {
      const conversationTitle = universityContext ? `${universityContext.name} Chat` : 'New Chat';
  ...
  // CRITICAL FIX: Get current conversation messages with forced refresh
  const { messages: allMessages } = useAppStore();
  const currentMessages = currentConversation 
    ? allMessages.filter(msg => msg.conversationId === currentConversation.id)
    : [];
  ...
  const handleUniversitySelect = (university: { name: string; fullName: string; logo?: string }) => {
    ...
    // Create a new conversation for the selected university
    const conversationTitle = `${university.name} Chat`;
    const newConversationId = stableCreateConversation(conversationTitle);
  ...
  const handleStartNewConversation = () => {
    ...
    const newConversationId = startNewConversation('New Chat');
